...
if [ "$color_prompt" = yes ]; then
    PS1='${debian_chroot:+($debian_chroot)}\[\033[01;32m\]\u@$(lower_host)\[\033[00m\]:\[\033[01;34m\]$(git_branch)\w\[\033[00m\]\$ '
else
    PS1='${debian_chroot:+($debian_chroot)}\u@$(lower_host):$(git_branch)\w\$ '
fi
...
case "$TERM" in
xterm*|rxvt*)
    PS1="\[\e]0;${debian_chroot:+($debian_chroot)}\u@$(lower_host): \w\a\]$PS1"
    ;;
*)
    ;;
esac
...
for dir in /usr/local/share/npm/bin; do
    if [ -d $dir ]; then
        PATH=$dir:$PATH
    fi
done
